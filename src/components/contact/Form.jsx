...
const Form = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const mailtoUrl = `mailto:adharashivkar17@gmail.com?subject=${subject}&body=${body}`;

    try {
      // Try FormSubmit first
      const tempForm = document.createElement('form');
      tempForm.action = 'https://formsubmit.co/adharashivkar17@gmail.com';
  ...
  const handleMailtoFallback = () => {
    ...
    window.location.href = `mailto:adharashivkar17@gmail.com?subject=${subject}&body=${body}`;
  };
    ...
    <div>
      ...
      <div className="mx-2">
        <form className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-lg:items-center mt-5">
            <button
              type="submit"
              disabled={isSubmitting}
              className={`btn gap-3 btn-primary rounded-sm text-[13px] md:text-[16px] w-fit font-semibold lg:mt-12.5 p-2 md:px-4 ${
                isSubmitting ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
              {isSubmitting ? 'Sending...' : 'Submit'} {!isSubmitting && telegramSVG}
            </button>
            
            <button
              type="button"
              onClick={handleMailtoFallback}
              className="btn gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 border-gray-300 rounded-sm text-[13px] md:text-[16px] w-fit font-semibold lg:mt-12.5 p-2 md:px-4"
            >
              📧 Direct Email
            </button>
          </div>
